# Ejercicios propuestos en https://josejuansanchez.org/bd/ejercicios-consultas-sql/index.html#gesti%C3%B3n-de-empleados
USE jardineria;
...
# 1.- Devolver un listado con el código de oficina y la ciudad donde hay oficinas
...
# Devuelve el nombre de los clientes que han hecho pagos y el nombre de sus representantes junto con 
...
SELECT
	c.nombre_cliente,
    e.nombre AS 'Nombre rep. de venta',
...
FROM 
	empleado e1
		LEFT JOIN
	empleado e2 ON e2.codigo_empleado =e1.codigo_jefe;
...
SELECT
	...
	e3.apellido1 AS apellido_jefe_jefe    
FROM 
	empleado e1
		LEFT JOIN
	empleado e2 ON e2.codigo_empleado =e1.codigo_jefe
		LEFT JOIN
	empleado e3 ON e3.codigo_empleado =e2.codigo_jefe;
...
FROM
	...
	detalle_pedido d ON p.codigo_pedido = d.codigo_pedido
		JOIN
	producto pd ON pd.codigo_producto= d.codigo_producto
		JOIN
	gama_producto g ON g.gama = pd.gama;
...
SELECT
	COUNT(codigo_empleado) AS 'Número de empleados'
FROM
	empleado;
    
# ¿Cuántos clientes tiene cada país?
SELECT
	COUNT(codigo_cliente) AS 'Num de clientes',
	pais
FROM
	cliente
GROUP BY pais;
